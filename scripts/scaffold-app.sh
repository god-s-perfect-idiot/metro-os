...
echo "Next (Reference research / Phase 0 — see AGENTS.md, do BEFORE coding):"
echo "      1. Write references/guides/blueprint.md (all pages)"
echo "      2. Populate references/web-resources.md with real WP8.1 sources"
echo "      3. Prefill references/images/ with a capture per page (or log references/known-gaps.md)"
echo "      4. Then add Gradle project under apps/$NAME/app/ and implement"
echo "Hub catalog (before first release sync):"
echo "      5. Add metro_app_${NAME}.xml + MetroAppGlyphs + glyphFiles in sync-hub-firestore.sh"
echo "         (sync refuses first-party upserts without logoXml — see docs/HARNESS.md)"