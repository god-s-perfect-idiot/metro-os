...
- Long-press tile edit overlay: resize (cycles small/medium/wide when `BuildConfig.WIDE_TILES`), unpin, brush customize (background + optional App Widget), and drag-to-reorder (tile follows thumb; magnet reflow; order persisted on drop)
...
- Cold start shows `MetroSplashLoadingScreen` (accent splash + Start glyph + dancing dots): system splash stays until the View-backed dots start, then Compose holds until Start paints and dots have had a visible beat (~700ms). Live tile providers fill in afterward without re-covering Start; resume refreshes also update in place — no splash. Unlock / screen-off skips the Start enter wave. Settings / theme observers never resolve live tile ContentProviders on the main thread (was ANR on unlock).
...
- When a package has multiple active notifications, the live tile cycles through each peek face one flip at a time (newest first); the badge still shows the total count
