...
- **Interactions:**
  - **Fingerprint / Face unlock:** Pass through to the system. On successful biometric unlock, the Metro fill **animates a swipe-up slide-off**, then removes (`USER_PRESENT` / keyguard unlocked). Do not open SystemUI lock input after biometric success.
  - **Swipe up:** Lock fill **tracks the finger** upward. Release **below** the unlock threshold → spring bounce back (no SystemUI). Release **at or past** threshold → panel animates fully off-screen, then SystemUI’s **lock input** (PIN / pattern / password) is shown — not the decorative lock wallpaper. Prefer an accessibility swipe-up on the keyguard after the Metro fill is removed; fall back to `KeyguardManager.requestDismissKeyguard` (transparent trampoline) only when the gesture cannot be dispatched. After a committed swipe, the Metro fill stays suppressed until the next screen-off / lock — biometric fail or cancel must not restore it.
