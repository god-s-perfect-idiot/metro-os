...
Mirror WP8.1 settings: root `system` | `applications` pivot (all launchable apps + in-Settings app detail with toggles/uninstall), start+theme accent colour picker (20 WP8 colours) + chrome font ListPicker (Metro Noto / Source Sans 3 / Alegreya Sans) + icon pack drill-in picker (`MetroIconPackPickerScreen`) + Start background choose photo, ease of access text size (10-step: WP8.1's 7 plus three smaller steps), brightness, storage sense, connected apps (gallery / music live-tile package lists), shell setup rows (launch `com.metro.navbar` / `com.metro.statusbar` / `com.metro.notifications` / `com.metro.volume` / `com.metro.lockscreen`), keyboard (launch `com.metro.keyboard`), and about (more info device details). Never deep-link into the Android Settings app.
