...
object NavbarOverlayController {
    ...
    fun activate(context: Context) {
        ...
        displayRotation = readDisplayRotation(context.applicationContext)
    ...
    fun onConfigurationChanged(newConfig: Configuration) {
        ...
        val nextRotation = readDisplayRotation(context)
    ...
    fun applyBackgroundModePreference() {
        navbarState?.applyBackgroundModePreference()
    }

    fun applyVisibilityMode(mode: NavbarVisibilityMode) {
        navbarState?.applyVisibilityMode(mode)
    }

    fun applySystemNavigationBarsHidden(hidden: Boolean) {
        navbarState?.applySystemNavigationBarsHidden(hidden)
    }
    ...
    fun applyForegroundPackage(
        ...
        navbarState?.applyForegroundPackage(packageName, epoch, authoritative, fromProbe)
    }

    fun currentForegroundEpoch(): Long = navbarState?.foregroundEpochPeek() ?: 0L
    ...
    private fun show(context: Context, windowType: Int) {
        ...
        val composeView = ComposeView(context).apply {
            ...
            setContent {
                NavbarOverlaySurface(
                    state = state,
                    displayRotation = displayRotation,
                    onRotateRelayout = { /* layout params stay MATCH_PARENT; rotation is visual */ },
    ...
    private fun readDisplayRotation(context: Context): Int {
