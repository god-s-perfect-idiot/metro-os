...
- Toast: accent fill, square logo + one truncated line; no clock (tray already shows time); 3/5/10s timeout (setup ListPicker, default 5s), swipe right to dismiss, tap to open
...
- Stock Android heads-up disabled while the overlay FGS runs (`heads_up_notifications_enabled=0`)
